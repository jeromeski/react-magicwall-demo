...
*/
	(function ($, document, window) {
		...
		function setSize(size, dimension) {
			return Math.round(
				(/%/.test(size) ? (dimension === "x" ? $window.width() : winheight()) / 100 : 1) *
					parseInt(size, 10)
			);
		}
		...
		publicMethod.position = function (speed, loadedCallback) {
			...
			if (settings.get("right") !== false) {
				...
				left += Math.round(
					Math.max($window.width() - settings.w - loadedWidth - interfaceWidth, 0) / 2
				);
			}
			...
			if (settings.get("bottom") !== false) {
				...
				top += Math.round(
					Math.max(winheight() - settings.h - loadedHeight - interfaceHeight, 0) / 2
				);
			}
		...
		publicMethod.prep = function (object) {
			...
			callback = function () {
				...
				if (total > 1) {
					...
					$next[settings.get("loop") || index < total - 1 ? "show" : "hide"]().html(
						settings.get("next")
					);
